...
import { TrendingUp, Award, Clock, BarChart2 } from 'lucide-react';
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-popover border border-border rounded-xl px-3 py-2 shadow-elevated">
        ...
        <p className="text-sm font-bold text-foreground">{formatHoursDisplay(payload[0].value)}</p>
      </div>
    );
  }
  return null;
...
export default function Analytics() {
  ...
  const stats = useMemo(() => {
    ...
    const hoursArr = allCalc.map(r => calculateDailyHours(r)).filter(h => h > 0);

    if (hoursArr.length === 0) {
      return { avgDaily: 0, avgWeekly: 0, highest: 0, lowest: 0, highestDate: '', lowestDate: '' };
    }
    ...
    const highestRecord = allCalc.find(r => calculateDailyHours(r) === highest);
    ...
    allCalc.forEach(r => {
      ...
      weeklyTotals[weekKey] = (weeklyTotals[weekKey] || 0) + calculateDailyHours(r);
    });
    const weeklyArr = Object.values(weeklyTotals);
    const avgWeekly = weeklyArr.length > 0 ? weeklyArr.reduce((a, b) => a + b, 0) / weeklyArr.length : 0;
    ...
    return {
      ...
      highest,
      lowest,
      highestDate: highestRecord ? format(parseDateKey(highestRecord.date), 'MMM d') : '',
      lowestDate: lowestRecord ? format(parseDateKey(lowestRecord.date), 'MMM d') : '',
    };
  }, [monthRecords]);
  ...
  return (
    <div className="page-enter px-4 py-5 space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">{format(today, 'MMMM yyyy')}</p>
      </div>
        ...
        <>
            ...
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyChartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.88 0.01 85 / 0.3)" vertical={false} />
                  <XAxis
                    dataKey="day"
          ...
          <div className="app-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-foreground">{format(today, 'MMMM')} by Week</h3>
            </div>
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="app-card p-4">
                ...
                <p className="text-xl font-display font-bold text-foreground">{formatHoursDisplay(stats.avgDaily)}</p>
              </div>
              <div className="app-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  <BarChart2 className="w-4 h-4 text-primary" />
                  <p className="text-xs text-muted-foreground font-medium">Avg Weekly</p>
                </div>
                <p className="text-xl font-display font-bold text-foreground">{formatHoursDisplay(stats.avgWeekly)}</p>
              </div>
              <div className="app-card p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xl font-display font-bold text-foreground">{formatHoursDisplay(stats.highest)}</p>
                {stats.highestDate && <p className="text-xs text-muted-foreground">{stats.highestDate}</p>}
              </div>
              <div className="app-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  <TrendingUp className="w-4 h-4 text-muted-foreground" />
                  <p className="text-xs text-muted-foreground font-medium">Lowest Day</p>
                </div>
                <p className="text-xl font-display font-bold text-foreground">{formatHoursDisplay(stats.lowest)}</p>
                {stats.lowestDate && <p className="text-xs text-muted-foreground">{stats.lowestDate}</p>}
              </div>
            </div>
          </div>
        </>
